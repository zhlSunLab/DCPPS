...
from sklearn.metrics import roc_curve, auc, roc_auc_score, matthews_corrcoef, f1_score, precision_score  ###计算roc和auc
...
def DeepNet_training(training_set_name, X_train_positive, X_train_negative, global_train_positive, global_train_negative, Y, X_val1, X_val2,Y_val,modelname,t):
    ...
    for i in range(num):
        ...
        filepath2 = filepath1 + '/' + 'weights%d-demo%s.h5' % ((i + 1),t)
...
def DeepNet_testing(X_test, X_test_2,Y, modelname,X_val1, X_val2,Y_train_val,t):
    ...
    roc_auc = auc(fpr, tpr)
    F1 = f1_score(Y_test, predict_, pos_label=0)
    Precision = precision_score(Y_test, predict_, pos_label=0)
    recalls = recall_score(Y_test, predict_, pos_label=0)
    TP, TN, FP, FN = metric_10(Y_test, predict_)
    Sp = float(TN) / float(TN + FP)
    acc = accuracy_score(Y_test, predict_)
    mcc = matthews_corrcoef(Y_test, predict_)
    precision, recall, thresholds = precision_recall_curve(Y_test, predict_probe_[:, 0], pos_label=0)
    auc_pr = auc(recall, precision)
    print('The %dth test result of %s is:\nauc of roc:%.4f\nauc of pr:%.4f\nPrecision is %.4f\nF1 is %.4f\nAcc is %.4f\nSp is %.4f' % (t, modelname, roc_auc, auc_pr, Precision, F1, acc, Sp))

    return predict_, predict_probe_[:, 0], Y_test, roc_auc, auc_pr, fpr, tpr, Precision, recalls, F1, acc, Sp, mcc
